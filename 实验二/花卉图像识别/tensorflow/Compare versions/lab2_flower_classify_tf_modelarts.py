...
# model_vgg16.load_weights(r'mobilenet_v2_weights_tf_dim_ordering_tf_kernels_1.0_224_no_top.h5')
...
opt = optimizers.Adam(learning_rate=0.01)   #使用Adam优化器，优化模型参数。lr(learning rate, 学习率)
...
model.fit(x_train, y_train, epochs=80, validation_data=(x_val, y_val),batch_size=200, verbose=2)
#输出模型的结构和参数量
model.summary()
